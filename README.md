...
| 2 | Пользователи | **Гостевая загрузка** без регистрации; опционально **аккаунт** (телефон РФ +7, пароль, имя) — JWT 7d в `sessionStorage`, при загрузке с Bearer к видео привязывается **имя автора** (snapshot). Регистрация/вход: `/register`, `/login`; профиль и «Мои видео» — только для авторизованных. |
| 3 | Формат и размер | Вход: **MP4, MOV, MKV, WebM, AVI** (до **1 ГБ**); выход для плеера: **MP4 H.264 + AAC** (конвертация в фоне). |
| 4 | Хранение файлов | **Локальный диск** на машине с API. |
| 5 | Обработка | **BullMQ** + воркер **ffmpeg**; статусы **`not_ready`** → **`ready`**; исходники в `uploads/sources/`. |
...
| 11 | Название видео | **Название** (до 75 символов) и **описание** (до 300) при загрузке; владелец может править и скрыть ролик (`visibility: private`). |
...
- Только сценарии публичного сайта: загрузка, метаданные, отдача файла с **Range** по **`publicId`**; **auth** (`/auth/register`, `/auth/login`, `/me`, `/me/videos`).
- **CORS** `*` (как зафиксировано); JWT в заголовке `Authorization: Bearer …`.
...
- В `.env`: **`USER_PUBLIC_SITE_URL`**, **`REDIS_URL`**, **`JWT_SECRET`**, **`JWT_EXPIRES_IN`** (по умолчанию `7d`).
...
- Форма загрузки: файл + **название**, прогресс, итоговая публичная ссылка (с Bearer — видео с автором).
- Плеер: `<video controls>`, заголовок страницы из **названия**, блок **«Автор»** если есть.
- **Вход / регистрация / профиль / мои видео** — маршруты `/login`, `/register`, `/profile`, `/my/videos`.
