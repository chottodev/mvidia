...
- **Скринкаст** (`/screencast`): запись **экрана** (основа) + опционально **вебкамера** (PiP) и **микрофон**; превью и выбор устройств, настройки в `localStorage`. **Chrome / Edge (desktop)**, **HTTPS** / localhost / 127.0.0.1 → `POST /videos` (MP4 + H.264, при микрофоне — AAC если поддерживает браузер). Подробности — `docs/screencast-recording-plan.md`.
